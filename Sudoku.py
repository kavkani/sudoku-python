from ursina import *
import very_easy_table
import Solver
app = Ursina()


def enable(bx, t=0):
    b_list[bx].color = color.black
    if t != 0:
        b_list[bx].text = t
    for item in b_list:
        if item.color == color.black:
            item.disabled = False
    for num in nums:
        num.disabled = True
    deselect.disabled = True


def click(b, bx):
    global nums, b_list, deselect
    if b.color == color.black:
        for item in b_list:
            if item != b_list[bx]:
                item.disabled = True
        b_list[bx].color = color.yellow
        for num in nums:
            num.disabled = False
            num.on_click = Func(enable, bx, num.text)
        deselect.disabled = False
        deselect.on_click = Func(enable, bx)
    return 0


b_list = []
mic = Button(icon="mic", scale=0.06, position=(0.6, -0.26), color=color.blue, disabled=True)
deselect = Button(icon="deselect1", scale=0.06, position=(0.6, -0.34), color=color.blue, disabled=True)
check = Button(text="Check", position=(0.59, -0.44), scale=(0.15, 0.1), color=color.orange)
y = 0.46
nums = []
for i in range(9):
    number = Button(text=str(i + 1), scale=0.06, position=(0.6, y), color=color.blue, disabled=True)
    nums.append(number)
    y -= 0.08
y = 0.443
final = very_easy_table.generate_table()
for i in range(9):
    x = -0.53
    for j in range(9):
        button = Button(scale=0.105, position=(x, y), highlight_color=color.lime, color=color.black)
        if final[i][j] != 0:
            button.text = str(final[i][j])
            button.color = color.dark_gray
            button.disabled = True
        b_list.append(button)
        button.on_click = Func(click, button, i * 9 + j)
        if (j + 1) % 3 == 0:
            x += 0.114
        else:
            x += 0.105
    if (i + 1) % 3 == 0:
        y -= 0.114
    else:
        y -= 0.105
window.fullscreen = True
Sky(texture="bg")
app.run()
